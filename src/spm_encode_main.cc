...
ABSL_FLAG(std::string, output_format, "piece",
          "choose from piece, cased_piece, id, proto, nbest_piece, nbest_id, nbest_proto, "
          "sample_piece, cased_sample_piece, sample_id or sample_proto.");
...
int main(int argc, char *argv[]) {
  ...
  } else if (absl::GetFlag(FLAGS_output_format) == "cased_piece") {
    process = [&](const std::string &line) {
      CHECK_OK(sp.EncodeSurface(line, &sps));
      output->WriteLine(absl::StrJoin(sps, " "));
    };
  ...
  } else if (absl::GetFlag(FLAGS_output_format) == "cased_sample_piece") {
    process = [&](const std::string &line) {
      CHECK_OK(sp.SampleEncodeSurface(line, nbest_size, alpha, &sps));
      output->WriteLine(absl::StrJoin(sps, " "));
    };
